...
temas := [
    ...
    { nombre:"Negro Dorado", fondo:"080600", texto:"FFD700", barra:"1A1200", textoBarra:"FFD700", historial:"050400", panel:"120E00", cooldown:"FF4422", afk:"FFC200", boton:"1A1200", hover:"2E2000", logo:"FFD700", luzOn:"CCA800", luzAccion:"FFD700", luzOff:"080600", btnTexto:"FFD700", histColor1:"FFD700", histColor2:"CCA800", histColor3:"FF9500" },
...
pasosNormales.Push({ tipo:"pimg", nombre:"play",          color:0xFFD510, categoria:2, accion:"c", modo:"tecla", tolerancia:1, hold:400, delayTecla:500, cooldown:500, lastUsed:0, x1:37, y1:271, x2:37, y2:271 })
...
pasosNormales.Push({ tipo:"pimg", nombre:"gamedone1",     color:0x000033, categoria:4, accion:"c", hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:3000, lastUsed:0, x1:941, y1:40, x2:959, y2:43 })
pasosNormales.Push({ tipo:"pimg", nombre:"gamedone2",     color:0xF7F9F9, categoria:4, accion:"c", hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:3000, lastUsed:0, x1:900, y1:43, x2:900, y2:43 })
pasosNormales.Push({ tipo:"pimg", nombre:"gamedone3",     color:0xF7F9F9, categoria:4, accion:"c", hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:3000, lastUsed:0, x1:876, y1:51, x2:876, y2:51 })
...
pasosNormales.Push({ tipo:"pimg", nombre:"leaving...",    color:0x30F1DD, categoria:5, hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, lastUsed:0, x1:859, y1:928, x2:863, y2:931 })
...
CargarStats() {
    global statsPath, totalHorasGuardadas, totalSecuenciasGuardadas
...
MostrarEstadisticas(*) {
    global totalHorasGuardadas, totalSecuenciasGuardadas
    global tiempoAcumulado, tiempoInicio, timerActivo, contadorSecuencias
    ...
    statsGui.Show("w280 h112 Center")  
    btnExp := statsGui.Add("Button", "x20 y116 w240 h24 Background" colorBotonNormal, Chr(128190) " Exportar sesión")
    btnExp.SetFont("s9 c" colorBtnTexto " Bold", "Segoe UI")
    btnExp.OnEvent("Click", ExportarSesion)
    statsGui.Show("w280 h148 Center") 
...
WM_MOUSEMOVE_Handler(wParam, lParam, msg, hwnd) {
    global btnIniciar, btnParar, btnCodigo, btnReset, btnHistorial, btnTema, btnMin, btnClose, btnCompacto
    global colorBotonHover, rgbBotones, colorRGBActual
    for btn in [btnIniciar, btnParar, btnCodigo, btnReset, btnHistorial, btnTema, btnMin, btnClose, btnCompacto] {
        if (hwnd = btn.Hwnd) {
...
; ===== FUNCIONES GUI =====
LeerTemaGuardado() {
    global configPath, temas, eggDesbloqueado
...
ExportarSesion(*) {
    ...
    txt .= "Sin wifi     : " contadorSinWifi "`n"
    txt .= "══════════════════════════════`n"
...
ActualizarAFK(*) {
    global ultimoCambio, afkText, colorAFK, rgbActivo
...
Parar(*) {
    global activo, accionEnCurso, bloqueoGlobalHasta
    activo := false
    accionEnCurso := false
...
ComprobarVersionRemota(lblVer, lblEstado, btnAct) {
    ...
    try {
        ...
        whr.Open("GET", GITHUB_VERSION_URL, false)
        whr.Send()
        if (whr.Status != 200) {
...
DescargarYActualizar(verRemota, lblEstado, btnAct) {
    global GITHUB_SCRIPT_URL, updateGui, updateGuiVisible
    ...
    try {
        whr := ComObject("WinHttp.WinHttpRequest.5.1")
        whr.Open("GET", GITHUB_SCRIPT_URL, false)
        whr.Send()
        if (whr.Status != 200) {
        ...
        rutaDestino := A_ScriptFullPath
        rutaTemp    := A_ScriptDir "\~macro_update_temp.ahk"

        ; Escribe en temporal primero para no corromper si falla
        FileDelete(rutaTemp)
        FileAppend(whr.ResponseText, rutaTemp, "UTF-8")

        ; Reemplaza el script actual
        FileCopy(rutaTemp, rutaDestino, 1)
        FileDelete(rutaTemp)
...
; Tab + A = ventana actualizador
~Tab & a::AbrirVentanaActualizacion()
