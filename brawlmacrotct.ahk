...
pasosNormales.Push({ tipo:"pimg", nombre:"glitch1",       color:0x0059A2, categoria:3, accion:"Esc", hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, lastUsed:0, x1:38, y1:252, x2:53, y2:259 })
pasosNormales.Push({ tipo:"pimg", nombre:"featured",      color:0x0E2C45, categoria:3, accion:"Esc", hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, lastUsed:0, x1:166, y1:262, x2:166, y2:269 })
...
pasosNormales.Push({ tipo:"pimg", nombre:"closing...",    color:0xD7D554, categoria:5, noPrioridad:true, hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:300000, lastUsed:0, x1:742, y1:515, x2:743, y2:518 })
pasosNormales.Push({ tipo:"pimg", nombre:"enteringroom1", color:0xFF89D0, categoria:5, noPrioridad:true, hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, lastUsed:0, x1:389, y1:566, x2:393, y2:567 })
pasosNormales.Push({ tipo:"pimg", nombre:"enteringroom2", color:0x3F7F96, categoria:5, noPrioridad:true, hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, lastUsed:0, x1:366, y1:549, x2:366, y2:549 })
pasosNormales.Push({ tipo:"pimg", nombre:"leaving...",    color:0x30F1DD, categoria:5, noPrioridad:true, hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, bloqueoGlobal:3000, lastUsed:0, x1:859, y1:928, x2:863, y2:931 })
...
pasosNormales.Push({ tipo:"pimg", nombre:"offline",       color:0x3F4967, categoria:6, noPrioridad:true, tiempoNecesario:300000, tiempoDetectando:0, hold:400, tolerancia:1, delayClick:500, delayTecla:500, cooldown:500, lastUsed:0, x1:1547, y1:99, x2:1547, y2:99 })
...
global ultimaDeteccion := 0          ; tick de la última vez que se detectó algún paso normal
...
; ===== HELPER: ejecutar un paso detectado =====
_EjecutarPasoDetectado(paso, x, y) {
    global activo, ultimoCambio, ultimoPasoEjecutado, tiempoUltimoLanzamiento
    global modoCadena, pasoCadena, finCadena, contadorEsc, contadorSinWifi, ultimaDeteccion

    contadorEsc := 0
    modo := paso.HasProp("modo") ? paso.modo : "click+tecla"
    MouseMove(x, y, 5)
    tiempoUltimoLanzamiento := 0
    ultimaDeteccion := A_TickCount   ; se detectó algo → resetear timer noPrioridad
...
EjecutarMacro(*) {
    ...
    ; ── Constantes de la nueva lógica ──────────────────────────────────
    static PASOS_ENTRE_PRIO  := 5       ; check prioridad cada N pasos normales
    static TIMEOUT_NO_PRIO   := 20000   ; ms sin detección para revisar noPrioridad
    ...
    ; ── ¿Hace más de 20 s sin detectar nada? → revisar noPrioridad ────
    ; (bloqueo global sigue siendo respetado — _EjecutarPasoDetectado
    ;  llama a ActivarBloqueoGlobal y este if solo llega si !BloqueoGlobalActivo)
    tiempoSinDeteccion := A_TickCount - ultimaDeteccion
    if (tiempoSinDeteccion >= TIMEOUT_NO_PRIO) {
        for paso in pasosNormales {
            if !paso.HasProp("noPrioridad") || !paso.noPrioridad
                continue
            if !paso.HasProp("lastUsed")
                paso.lastUsed := 0
            if paso.HasProp("cooldown") && (A_TickCount - paso.lastUsed < paso.cooldown)
            ...
            if BuscarPixel(paso, &x, &y) {
                if paso.HasProp("tiempoNecesario") {
                    ...
                    if (A_TickCount - paso.tiempoDetectando < paso.tiempoNecesario) {
                        accionEnCurso := false
                        return
                    }
                } else {
    ...
    ; ── Loop principal (solo pasos normales, sin noPrioridad) ─────────
    ; CheckPrioridad se llama cada PASOS_ENTRE_PRIO pasos revisados
    pasoRevisado := 0
    for paso in pasosNormales {
        ; Saltar pasos noPrioridad en el loop normal
        if paso.HasProp("noPrioridad") && paso.noPrioridad
            continue
        ...
        pasoRevisado += 1
        ; Cada 5 pasos revisados → checkear prioridad
    ...
    ; ===== MODO DESTRUCCION =====
    ; Se activa si llevamos mas de 360s (300s AFK + 1 minuto extra) sin cambio de patron
    ...
    ; Si se lanzó Brawlhalla pero no se detecta nada en 2 minutos, volver a intentarlo
...
Iniciar(*) {
    global activo, ultimoCambio, modoDestruccion, ultimoPasoEjecutado, ultimaDeteccion
    activo := true
    ultimoCambio := A_TickCount
    ultimaDeteccion := A_TickCount   ; resetear timer noPrioridad al iniciar
    modoDestruccion := false
    ultimoPasoEjecutado := ""
    ActualizarEstadoVisual()
